...
int ASerialPacket::GetMode(void)
{
    return m_mode;
}
...
uint16_t ASerialPacket::GetLastErrorCode(void)
{
    return m_lase_error_code;
}
...
void ASerialPacket::SetConnectionState(bool state)
{
...
int ASerialPacket::MakePacketData(uint8_t *to_device_data, int data_num, uint8_t command, uint8_t *data_packet_out)
{
    ...
    if (GetConnectionState() == false && command != RESERVED_COMMAND_GET_INFO &&
        command != RESERVED_COMMAND_RESET) { // 接続状態がfalseかつコマンドがリセットやデバイス情報リクエストでない場合はエラー
        return -1;
    }
