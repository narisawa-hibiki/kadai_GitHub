...
#ifndef ASERIAL_PACKET_H
#define ASERIAL_PACKET_H
...
namespace ASerialDataStruct {
  ...
  } ToDeviceData; //デバイスへのデータ構造体

  typedef struct {
    uint8_t data_num = 0;
    uint8_t data[_ASERIAL_MAX_DATA_NUM] = {0};
    uint16_t check_data = 0;
  } ToControllerData; //コントローラへのデータ構造体
